import { _ as _defineProperty, Q as QuantitySchema, A as AmountSchema, B as BytesLikeSchema, N as NFTInputOrUriSchema, a as BasisPointsSchema, F as FileOrBufferOrStringSchema, C as CommonNFTOutput, b as CommonNFTInput, D as DEFAULT_QUERY_ALL_COUNT, c as BasicNFTInput, M as MAX_BPS, P as PercentSchema } from './QueryParams-fadca675.browser.esm.js';
import { z } from 'zod';
import { BigNumber, providers, utils, constants, Contract } from 'ethers';
import { isFileOrBuffer, replaceGatewayUrlWithScheme } from '@thirdweb-dev/storage';
...
import { defaultChains, getValidChainRPCs } from '@thirdweb-dev/chains';
...
class ContractWrapper extends RPCConnectionHandler {
  ...
  async formatError(error, fn, args, callOverrides) {
    ...
    let contractName = undefined;
    try {
      const metadata = await fetchContractMetadataFromAddress(this.readContract.address, this.getProvider(), this.storage);
      if (metadata.name) {
        contractName = metadata.name;
      }
      if (metadata.metadata.sources) {
...
class Erc721 {
  ...
  constructor(contractWrapper, storage, chainId) {
    ...
    _defineProperty(this, "loyaltyCard", void 0);
    _defineProperty(this, "updatableMetadata", void 0);
    _defineProperty(this, "contractWrapper", void 0);
    _defineProperty(this, "storage", void 0);
    _defineProperty(this, "_chainId", void 0);
    /**
...
const SignerWithRestrictionsSchema = /* @__PURE__ */z.object({
  signer: AddressOrEnsSchema,
  isAdmin: /* @__PURE__ */z.boolean(),
  restrictions: AccessRestrictionsSchema
});
const SignerWithRestrictionsBatchSchema = /* @__PURE__ */z.array(SignerWithRestrictionsSchema);
...
const EditionDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [abi, contract, _network] = await Promise.all([await EditionDropInitializer.getAbi(address, provider, storage), import('./edition-drop-1957b07b.browser.esm.js'), provider.getNetwork()]);
...
const EditionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([EditionInitializer.getAbi(address, provider, storage), import('./edition-3a6ab922.browser.esm.js'), provider.getNetwork()]);
...
const MarketplaceInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceInitializer.getAbi(address, provider, storage), import('./marketplace-18e0dc79.browser.esm.js'), provider.getNetwork()]);
...
const MarketplaceV3Initializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MarketplaceV3Initializer.getAbi(address, provider, storage), import('./marketplacev3-bd574272.browser.esm.js'), provider.getNetwork()]);
...
const MultiwrapInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([MultiwrapInitializer.getAbi(address, provider, storage), import('./multiwrap-be4eeae1.browser.esm.js'), provider.getNetwork()]);
...
const NFTCollectionInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTCollectionInitializer.getAbi(address, provider, storage), import('./nft-collection-5c071bed.browser.esm.js'), provider.getNetwork()]);
...
const NFTDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([NFTDropInitializer.getAbi(address, provider, storage), import('./nft-drop-20f92611.browser.esm.js'), provider.getNetwork()]);
...
const PackInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([PackInitializer.getAbi(address, provider, storage), import('./pack-7fb73caf.browser.esm.js'), provider.getNetwork()]);
...
const SignatureDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SignatureDropInitializer.getAbi(address, provider, storage), import('./signature-drop-69284f4b.browser.esm.js'), provider.getNetwork()]);
...
const SplitInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([SplitInitializer.getAbi(address, provider, storage), import('./split-5d7c0916.browser.esm.js'), provider.getNetwork()]);
...
const TokenDropInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenDropInitializer.getAbi(address, provider, storage), import('./token-drop-a6b097ca.browser.esm.js'), provider.getNetwork()]);
...
const TokenInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([TokenInitializer.getAbi(address, provider, storage), import('./token-00385864.browser.esm.js'), provider.getNetwork()]);
...
const VoteInitializer = {
  ...
  initialize: async function () {
    ...
    const [, provider] = getSignerAndProvider(network, options);
    const [abi, contract, _network] = await Promise.all([VoteInitializer.getAbi(address, provider, storage), import('./vote-b114afd4.browser.esm.js'), provider.getNetwork()]);
...
class Account {
  constructor(contractWrapper) {
    ...
    _defineProperty(this, "grantAccess", /* @__PURE__ */buildTransactionFunction(async (signerAddress, restrictions) => {
      ...
      const roleRestrictions = {
        ...
        maxValuePerTransaction: utils.parseEther(parsedRestrictions.nativeTokenLimitPerTransaction),
     ...
     * @twfeature Account
     */
    _defineProperty(this, "setAccess", /* @__PURE__ */buildTransactionFunction(async permissionsSnapshot => {
      /**
       * All cases
       *
       * - Add new admin :check:
      ...
      for (const currentAdmin of allCurrentAdmins) {
        if (!allNewAdmins.includes(currentAdmin)) {
          encoded.push(this.contractWrapper.readContract.interface.encodeFunctionData("setAdmin", [currentAdmin, false]));
        }
      }

      // All add-admin actions.
      const toRemoveAsScoped = {};
      for (const newAdmin of allNewAdmins) {
        if (!allCurrentAdmins.includes(newAdmin)) {
          const data = this.contractWrapper.readContract.interface.encodeFunctionData("setAdmin", [newAdmin, true]);
      ...
      for (const currentScoped of allCurrentScoped) {
        if (!allNewScoped.includes(currentScoped)) {
          const {
            payload,
            signature
          } = await this.generatePayload(currentScoped, RoleAction.REVOKE);
          encoded.push(this.contractWrapper.readContract.interface.encodeFunctionData("changeRole", [payload, signature]));
      ...
      for (const newScoped of allNewScoped) {
        if (!allCurrentScoped.includes(newScoped)) {
          ...
          const parsedRestrictions = await AccessRestrictionsSchema.parseAsync(permissionsSnapshot.find(result => result.signer === newScoped)?.restrictions);
          ...
          const {
            payload,
            signature
          } = await this.generatePayload(newScoped, RoleAction.GRANT);
          encoded.push(this.contractWrapper.readContract.interface.encodeFunctionData("changeRole", [payload, signature]));
        }
      }
      ...
      for (const currentScoped of allCurrentScoped) {
        if (allNewScoped.includes(currentScoped)) {
          ...
          const parsedRestrictions = await AccessRestrictionsSchema.parseAsync(permissionsSnapshot.find(result => result.signer === currentScoped)?.restrictions);
  ...
  async getFactory() {
    ...
    const wrapper = new ContractWrapper(chainId, factoryAddress, IAccountFactory, this.contractWrapper.options, this.contractWrapper.storage);
    wrapper.updateSignerOrProvider(this.contractWrapper.getSigner());
    return wrapper;
  }
  ...
  async getSignersWithRestrictions() {
    ...
    const signers = await contract.readContract.getSignersOfAccount(this.getAddress());
...
class AccountFactory {
   ...
   * @twfeature AccountFactory
   */
  async getAssociatedSigners(account) {
  ...
  async getAllAccounts() {
    const allAccounts = await this.contractWrapper.readContract.getAllAccounts();
     ...
     */
    return await Promise.all(allAccounts.map(async account => {
...
class ContractPublisher extends RPCConnectionHandler {
  constructor(_network, options, storage) {
    ...
    this.storage = storage;
    this.publisher = new ContractWrapper(_network, getContractPublisherAddress(), ContractPublisherAbi, options, storage);
  }
  updateSignerOrProvider(network) {
    super.updateSignerOrProvider(network);
...
export { GenericRequest as $, AbiTypeSchema as A, BYOCContractMetadataSchema as B, CustomContractInput as C, DeployTypeInput as D, ExtraPublishMetadataSchemaInput as E, FactoryDeploymentSchema as F, CommonPlatformFeeSchema as G, CommonTrustedForwarderSchema as H, CommonSymbolSchema as I, CurrencySchema as J, CurrencyValueSchema as K, BaseSignaturePayloadInput as L, Signature20PayloadInput as M, Signature20PayloadOutput as N, Signature721PayloadInput as O, PreDeployMetadata as P, Signature721PayloadOutput as Q, Signature1155PayloadInput as R, StaticJsonRpcBatchProvider as S, Signature1155PayloadInputWithTokenId as T, Signature1155PayloadOutput as U, Signature721WithQuantityInput as V, Signature721WithQuantityOutput as W, MintRequest20 as X, MintRequest721 as Y, MintRequest1155 as Z, MintRequest721withQuantity as _, CustomContractOutput as a, ContractAppURI as a$, MerkleSchema as a0, SnapshotEntryInput as a1, SnapshotInputSchema as a2, SnapshotEntryWithProofSchema as a3, SnapshotSchema as a4, SnapshotInfoSchema as a5, BigNumberSchema as a6, BigNumberishSchema as a7, BigNumberTransformSchema as a8, AddressSchema as a9, Erc721Enumerable as aA, Erc721TieredDrop as aB, Erc721 as aC, Erc721WithQuantitySignatureMintable as aD, Erc721Burnable as aE, Erc1155BatchMintable as aF, Erc1155Burnable as aG, Erc1155Enumerable as aH, Erc1155LazyMintable as aI, Erc1155Mintable as aJ, Erc1155 as aK, Erc1155SignatureMintable as aL, MarketplaceV3DirectListings as aM, MarketplaceV3EnglishAuctions as aN, MarketplaceV3Offers as aO, GasCostEstimator as aP, ContractEvents as aQ, ContractInterceptor as aR, ContractPlatformFee as aS, ContractPublishedMetadata as aT, ContractOwner as aU, Transaction as aV, DeployTransaction as aW, defaultGaslessSendFunction as aX, biconomySendFunction as aY, defenderSendFunction as aZ, prepareGaslessRequest as a_, AddressOrEnsSchema as aa, RawDateSchema as ab, StartDateSchema as ac, EndDateSchema as ad, CallOverrideSchema as ae, ContractEncoder as af, ContractMetadata as ag, ContractRoles as ah, ContractRoyalty as ai, ContractPrimarySale as aj, DelayedReveal as ak, DropClaimConditions as al, DropErc1155ClaimConditions as am, Erc20BatchMintable as an, Erc20Burnable as ao, Erc20ClaimableWithConditions as ap, Erc20Droppable as aq, Erc20Mintable as ar, Erc20SignatureMintable as as, Erc20 as at, Erc721BatchMintable as au, Erc721ClaimableWithConditions as av, Erc721Claimable as aw, Erc721LazyMintable as ax, Erc721Mintable as ay, Erc721Supply as az, CustomContractDeploy as b, isExtensionEnabled as b$, Account as b0, AccountFactory as b1, AccessRestrictionsSchema as b2, SignerWithRestrictionsSchema as b3, SignerWithRestrictionsBatchSchema as b4, RoleAction as b5, RoleRequest as b6, Status as b7, ClaimEligibility as b8, isNativeToken as b9, RestrictedTransferError as bA, AdminRoleMissingError as bB, AuctionHasNotEndedError as bC, ExtensionNotImplementedError as bD, TransactionError as bE, parseRevertReason as bF, includesErrorMessage as bG, createSnapshot as bH, ALL_ROLES as bI, getRoleHash as bJ, fetchContractMetadataFromAddress as bK, fetchAbiFromAddress as bL, hasMatchingAbi as bM, extractConstructorParams as bN, extractFunctions as bO, extractCommentFromMetadata as bP, extractConstructorParamsFromAbi as bQ, extractFunctionsFromAbi as bR, extractEventsFromAbi as bS, extractMinimalProxyImplementationAddress as bT, resolveContractUriFromAddress as bU, extractIPFSHashFromBytecode as bV, fetchRawPredeployMetadata as bW, fetchPreDeployMetadata as bX, fetchExtendedReleaseMetadata as bY, detectFeatures as bZ, isFeatureEnabled as b_, cleanCurrencyAddress as ba, normalizePriceValue as bb, fetchCurrencyMetadata as bc, fetchCurrencyValue as bd, setErc20Allowance as be, approveErc20Allowance as bf, normalizeAmount as bg, toWei as bh, convertToReadableQuantity as bi, fetchSnapshotEntryForAddress as bj, NotFoundError as bk, InvalidAddressError as bl, MissingRoleError as bm, AssetNotFoundError as bn, UploadError as bo, FileNameMissingError as bp, DuplicateFileNameError as bq, NotEnoughTokensError as br, MissingOwnerRoleError as bs, QuantityAboveLimitError as bt, FetchError as bu, DuplicateLeafsError as bv, AuctionAlreadyStartedError as bw, FunctionDeprecatedError as bx, ListingNotFoundError as by, WrongListingTypeError as bz, CustomContractSchema as c, PREBUILT_CONTRACTS_MAP as c$, assertEnabled as c0, detectContractFeature as c1, hasFunction as c2, toSemver as c3, isIncrementalVersion as c4, isDowngradeVersion as c5, fetchSourceFilesFromMetadata as c6, CREATE2_FACTORY_BYTECODE as c7, SIGNATURE as c8, COMMON_FACTORY as c9, ChainId as cA, InterfaceId_IERC721 as cB, InterfaceId_IERC1155 as cC, NATIVE_TOKEN_ADDRESS as cD, NATIVE_TOKENS as cE, getNativeTokenByChainId as cF, LINK_TOKEN_ADDRESS as cG, EventType as cH, getChainProvider as cI, getChainIdFromNetwork as cJ, isChainConfig as cK, getProviderFromRpcUrl as cL, getSignerAndProvider as cM, SUPPORTED_FEATURES as cN, EditionDropInitializer as cO, EditionInitializer as cP, MarketplaceInitializer as cQ, MarketplaceV3Initializer as cR, MultiwrapInitializer as cS, NFTCollectionInitializer as cT, NFTDropInitializer as cU, PackInitializer as cV, SignatureDropInitializer as cW, SplitInitializer as cX, TokenDropInitializer as cY, TokenInitializer as cZ, VoteInitializer as c_, GAS_LIMIT_FOR_DEPLOYER as ca, DEPLOYER_BYTECODE as cb, DEPLOYER_ABI as cc, isContractDeployed as cd, isEIP155Enforced as ce, getCreate2FactoryAddress as cf, getSaltHash as cg, getInitBytecodeWithSalt as ch, computeDeploymentAddress as ci, computeEOAForwarderAddress as cj, computeForwarderAddress as ck, getKeylessTxn as cl, computeDeploymentInfo as cm, encodeConstructorParamsForImplementation as cn, getCreate2FactoryDeploymentInfo as co, fetchAndCachePublishedContractURI as cp, fetchAndCacheDeployMetadata as cq, resolveEns as cr, resolveAddress as cs, setSupportedChains as ct, getSupportedChains as cu, OZ_DEFENDER_FORWARDER_ADDRESS as cv, CONTRACT_ADDRESSES as cw, getContractAddressByChainId as cx, getContractPublisherAddress as cy, getMultichainRegistryAddress as cz, AbiObjectSchema as d, PREBUILT_CONTRACTS_APPURI_MAP as d0, CONTRACTS_MAP as d1, getContractTypeForRemoteName as d2, getContractName as d3, isProvider as d4, isSigner as d5, ContractWrapper as d6, getAllPluginsAbi as d7, caches as d8, CUSTOM_GAS_FOR_CHAIN as d9, TokenErc721ContractSchema as dA, DropErc721ContractSchema as dB, FEATURE_NFT_REVEALABLE as dC, FEATURE_PACK_VRF as dD, PackContractSchema as dE, PACK_CONTRACT_ROLES as dF, SplitsContractSchema as dG, ADMIN_ROLE as dH, DropErc20ContractSchema as dI, TOKEN_DROP_CONTRACT_ROLES as dJ, TokenErc20ContractSchema as dK, VoteContractSchema as dL, AddressZero as da, RPCConnectionHandler as db, signTypedDataInternal as dc, buildTransactionFunction as dd, ContractPublisher as de, SmartContract as df, getCompositePluginABI as dg, buildDeployTransactionFunction as dh, mapOffer as di, fetchTokenMetadataForContract as dj, isTokenApprovedForTransfer as dk, validateNewListingParam as dl, handleTokenApproval as dm, isWinningBid as dn, DropErc1155ContractSchema as dp, NFT_BASE_CONTRACT_ROLES as dq, TokenErc1155ContractSchema as dr, MarketplaceContractSchema as ds, MARKETPLACE_CONTRACT_ROLES as dt, FEATURE_DIRECT_LISTINGS as du, FEATURE_ENGLISH_AUCTIONS as dv, FEATURE_OFFERS as dw, MultiwrapContractSchema as dx, MULTIWRAP_CONTRACT_ROLES as dy, uploadOrExtractURI as dz, AbiSchema as e, ChainIdToAddressSchema as f, CustomFactoryInput as g, DeploymentNetworkInput as h, ExtraPublishMetadataSchemaOutput as i, FullPublishMetadataSchemaInput as j, FullPublishMetadataSchemaOutput as k, ProfileSchemaInput as l, ProfileSchemaOutput as m, PublishedContractSchema as n, ContractInfoSchema as o, CompilerMetadataFetchedSchema as p, PreDeployMetadataFetchedSchema as q, ClaimConditionMetadataSchema as r, ClaimConditionInputSchema as s, ClaimConditionInputArray as t, PartialClaimConditionInputSchema as u, ClaimConditionOutputSchema as v, CommonContractSchema as w, CommonContractOutputSchema as x, CommonRoyaltySchema as y, CommonPrimarySaleSchema as z };
